...
module.exports = defineConfig({
  ...
  webServer: {
    // ПРОДАКШЕН-СБОРКА, А НЕ `next dev`. Причина не в скорости, а в честности:
    // `next dev` собирает маршрут при первом обращении, и параллельные запросы,
    // попавшие в середину сборки, получают 500. Ошибка при этом приходит
    // из отрисовки страницы ошибки — «Cannot read properties of null (reading
    // useContext)» — и к нашему коду отношения не имеет.
    //
    // 13.08 это стоило получаса: витрина потяжелела на форму заявки, и три
    // теста начали падать через раз — вхолодную и только при нескольких
    // воркерах. Прогрев страниц не помог, помог только один воркер.
    //
    // Здесь тесты проверяют ровно то, что уезжает на Vercel. Плата — минута
    // сборки перед прогоном; она же ловит ошибки сборки раньше тестов.
